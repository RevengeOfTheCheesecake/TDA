...
The key finding emerged in \textbf{Section 7} when we discovered that \textbf{market segmentation} fundamentally determines topology stability:
...
This inequality transforms the empirical correlation-stability relationship ($\rho = -0.87$ between correlation and CV) into a \textbf{theoretically motivated pattern}, drawing on random matrix theory and spectral graph analysis.
...
\textbf{Explicit Limitations and Non-Claims}

To prevent misinterpretation, we explicitly state what this research does \textbf{not} demonstrate:

\begin{enumerate}
    \item \textbf{NOT a trading system}: This is a methodological study about \textit{when TDA works}, not a validated trading strategy ready for deployment

    \item \textbf{NOT a directional predictor}: AUC $\approx 0.52$ confirms topology cannot predict price direction---it detects \textit{when} regimes shift, not \textit{which way} prices move

    \item \textbf{NOT universal}: The $\rho_c \approx 0.50$ threshold is validated on US large-cap equities (2019--2024). It may not generalize to fixed income, commodities, emerging markets, or different time periods

    \item \textbf{NOT low-latency}: Persistent homology computation requires $O(n^4)$ time. For real-time applications, the Fiedler value proxy is recommended

    \item \textbf{NOT robust to small universes}: With $n < 15$ assets, correlation estimation noise dominates topological signal

    \item \textbf{NOT independent of correlation estimation}: All results depend on correlation matrix quality. The Epps effect, non-stationarity, and estimation error propagate into topology
\end{enumerate}

\textbf{Appropriate use cases}: Risk overlays, regime monitoring, volatility timing, portfolio construction constraints---\textit{not} standalone alpha generation.
}}
\end{center}
...
\begin{enumerate}
\item \checkmark \textbf{Among the first TDA strategies with positive out-of-sample performance} (Sharpe $+0.79$, risk-adjusted, net of costs, walk-forward validated)
\item \checkmark \textbf{Cross-market simulation study} (11 scenarios demonstrating theoretical generalization potential)
\item \checkmark \textbf{Rigorous TDA vs ML comparison} (feature importance, conservative AUC interpretation)
\item \checkmark \textbf{Theoretical bound} relating correlation to topology stability
\end{enumerate}

\textbf{Key Methodological Insight}: \textbf{Sector homogeneity appears critical}. Prior work computed topology on market-wide baskets (all stocks together), which our Section 7 results show produces unstable features (CV = 0.68). Computing topology \textbf{separately per sector} may be necessary to transform TDA from ``interesting visualization'' to ``tradeable signal.''
...
\textbf{What These Results Do NOT Imply:}
\begin{itemize}
\item[$\times$] \textbf{Not directional alpha}: Topology detects \textit{when} regimes shift, not \textit{which direction} prices move (AUC $\approx 0.52$)
\item[$\times$] \textbf{Not guaranteed profits}: Sharpe $+0.79$ is risk-adjusted and net of costs, but past performance $\neq$ future results
\item[$\times$] \textbf{Not universally applicable}: Only works when $\rho > 0.50$; fails on heterogeneous portfolios
\item[$\times$] \textbf{Not fully validated internationally}: Cross-market results use calibrated simulation, not live data
\end{itemize}
\textbf{Appropriate Use}: Risk overlay, regime detection, exposure scaling---\textit{not} standalone alpha generation.
}}
\end{center}
...
\textbf{More profound}: The correlation-stability mechanism shows consistency across 11 simulated market scenarios, three asset classes, and fiat-to-digital baskets. It is motivated by random matrix theory, supported by machine learning, and connected to spectral graph principles. This theoretical consistency suggests we have identified a \textbf{structural pattern} in networked systems that merits further empirical validation across live markets.
...
Three architectural improvements would strengthen this framework: (1) \textbf{Multi-horizon regime classifiers} using continuous ``regime intensity'' scores rather than binary classification; (2) \textbf{Fiedler value as real-time proxy}, enabling 50$\times$ faster regime detection for intraday applications; and (3) \textbf{Topology as meta-filter} for existing factor strategies (momentum, value, quality), leveraging its strength in regime detection while avoiding weak directional prediction. All code is available at \url{https://github.com/adam-jfkhs/TDA} for replication and extension.
