...
\textbf{Validation}: On held-out cross-market data (Section~\ref{sec:crossmarket}):
\begin{itemize}
\item \textbf{Precision}: 100\% (all predicted ghost regimes failed, Sharpe $< 0$)
\item \textbf{Recall}: 89\% (7/8 failed regimes correctly flagged ex ante)
\item \textbf{One false negative}: Emerging Markets ($\rho = 0.51$, borderline case)
\end{itemize}

\textbf{Practical Implication}: Practitioners can screen portfolios for ghost loop susceptibility using only correlation statistics, avoiding expensive persistent homology computation on unviable baskets.
...
\subsubsection{Informal Theorem}

\textbf{Theorem (Informal)}:

For a correlation matrix $\mathbf{C}$ with mean correlation $\rho \in [0, 1]$, the coefficient of variation of $H_1$ persistence values satisfies:
\begin{equation}
\text{CV}(H_1) \leq \frac{\alpha}{\sqrt{\rho(1 - \rho)}}
\label{eq:cv-bound}
\end{equation}
where $\alpha > 0$ is a constant depending on dimensionality $n$.
...
\begin{table}[H]
\centering
\caption{Empirical Validation of Theoretical Bound}
...
\textbf{Empirical Validation Across Markets}:
...
\item \checkmark \textbf{Empirical validation} across 11 markets (Sections 7--10)
\end{itemize}
...
\begin{enumerate}
\item \textbf{Bound Tightness}: The derived bound $\text{CV}(H_1) \leq \alpha/\sqrt{\rho(1-\rho)}$ is conservative (observed values 3--10$\times$ smaller). A tighter bound incorporating correlation dispersion $\sigma(\rho_{ij})$ and spectral gap $\Delta$ would improve predictive power.
...
\begin{enumerate}
\item \textbf{Out-of-Sample Regions}: Cross-market validation (Section~\ref{sec:crossmarket}) covers US, UK, Japan, and emerging markets. Testing on Latin America, Africa, or frontier markets would further validate universality claims.
